...
template HashChain(nInputs) {
    signal input inputs[nInputs];
    signal output hash;

    var current_state = 0; // Init value
    component poseidon[nInputs];
    for (var i = 0; i < nInputs; i++) {
        poseidon[i] = Poseidon(2);
        poseidon[i].inputs[0] <== current_state;
        poseidon[i].inputs[1] <== inputs[i];
        current_state = poseidon[i].out;
    }
